...
function classifyRisk(creditRating) {
...
export default function LeaseExpirationChart({ data }) {
  ...
  const chartData = useMemo(() => {
    ...
    // Build month buckets for all 24 months
    const buckets = {}
    for (let i = 0; i < 24; i++) {
  ...
  const maxRent = useMemo(() => Math.max(...chartData.map(d => d.total), 1), [chartData])
