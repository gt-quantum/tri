  ...
  return generator.generateDocument({
    ...
    info: {
      ...
      description: [
        ...
        '',
        'An MCP (Model Context Protocol) server is available for connecting AI clients like Claude Desktop, Claude Code, Cursor, and Cline to this API.',
        '',
        '### Quick Setup',
        '',
        '1. **Create an API key** with `manager` role (Settings > API Keys or `POST /api/v1/api-keys`)',
        '2. **Build the MCP server:**',
        '   ```',
        '   cd mcp && npm install && npm run build',
        '   ```',
        '3. **Configure your AI client** to run the server with your API key:',
        '   ```json',
        '   {',
        '     "mcpServers": {',
        '       "tri-real-estate": {',
        '         "command": "node",',
        '         "args": ["/path/to/tri/mcp/dist/index.js"],',
        '         "env": {',
        '           "TRI_API_KEY": "sk_live_...",',
        ...
        '   ```',
        '',
        'The MCP server runs locally on your machine (stdio transport) and proxies all requests through this API with `X-Change-Source: mcp`. See the `mcp/README.md` in the repository for detailed setup instructions for each client.',
      ].join('\n'),
    },
    servers: [{ url: '/' }],
