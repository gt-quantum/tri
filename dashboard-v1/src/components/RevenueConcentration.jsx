...
export default function RevenueConcentration({ data }) {
  const { leases, tenants } = data

  const { slices, metrics } = useMemo(() => {
    const activeLeases = leases.filter(l => l.status === 'active')

    // Aggregate rent by tenant
    const tenantRent = {}
    const tenantLeaseCount = {}
    ...
    // Sort tenants by rent descending
    const sorted = Object.entries(tenantRent)
      .map(([tid, rent]) => ({
        id: tid,
        name: tenants.find(t => t.id === tid)?.company_name || 'Unknown',
        value: rent,
    ...
    // Scale: 0–10,000. Under 1,500 = unconcentrated, 1,500–2,500 = moderate, above 2,500 = highly concentrated
