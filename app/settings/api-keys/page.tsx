...
export default function ApiKeysPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/v1/api-keys?limit=100&include_revoked=${includeRevoked}`,
        { headers }
      )
  ...
  async function handleRotate(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json()
        throw new Error(data.error?.message || 'Failed to rotate')
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-warm-300 font-body text-sm">
          Loading API keys...
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen p-6 max-w-5xl mx-auto">
                ...
                return (
                  ...
                  >
                    <td className="table-cell">
                      ...
                      {key.description && (
                        <div className="text-warm-400 text-xs mt-0.5 truncate max-w-[200px]">
                          {key.description}
                        </div>
                      )}
                    </td>
                    <td className="table-cell">
                      <code className="text-warm-300 text-xs bg-obsidian-800 px-1.5 py-0.5 rounded">
                        {key.key_prefix}...
                      </code>
                    </td>
                    <td className="table-cell">
                    ...
                    <td className="table-cell text-sm">
                      ...
                      >
                        {days <= 0
                          ? 'Expired'
                          : `${days}d (${formatDate(key.expires_at)})`}
                      </span>
                    </td>
                    <td className="table-cell">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-brass-faint">
                  ...
                  <th className="table-header">Key</th>
                  <th className="table-header">Role</th>
                  <th className="table-header">Expired</th>
                </tr>
              </thead>
              <tbody>
                {expiredKeys.map((key) => (
                  ...
                  >
                    ...
                    <td className="table-cell">
                      <code className="text-warm-500 text-xs">
                        {key.key_prefix}...
                      </code>
                    </td>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-brass-faint">
                  ...
                  <th className="table-header">Key</th>
                  <th className="table-header">Role</th>
                  <th className="table-header">Revoked</th>
                </tr>
              </thead>
              <tbody>
                {revokedKeys.map((key) => (
      ...
      {/* Key Display Modal (shown once after create/rotate) */}
      {showKeyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-obsidian-950/80 backdrop-blur-sm">
          <div className="card-surface p-6 w-full max-w-md mx-4">
            ...
            <p className="text-amber-300 text-sm font-body mb-4">
              Copy this key now. It will not be shown again.
            </p>

            <div className="relative">
              <code className="block w-full p-3 rounded-lg bg-obsidian-800 border border-brass-faint text-warm-white font-mono text-sm break-all select-all">
                {showKeyModal}
              </code>
              <button
                onClick={() => copyToClipboard(showKeyModal)}
                className="absolute top-2 right-2 px-2 py-1 rounded bg-obsidian-700 text-warm-300 text-xs font-body
                  hover:text-warm-white hover:bg-obsidian-600 transition-colors"
              >
                {copied ? 'Copied!' : 'Copy'}
              </button>
            </div>
            ...
            <div className="mt-4 p-3 rounded-lg bg-amber-500/5 border border-amber-500/10">
              <p className="text-warm-300 text-xs font-body">
                Use this key in the Authorization header:{' '}
                <code className="text-warm-200">
                  Authorization: Bearer {showKeyModal.slice(0, 12)}...
                </code>
              </p>
            </div>
            ...
            <button
              ...
              className="w-full mt-4 py-2.5 rounded-lg bg-brass text-obsidian-950 font-body font-semibold text-sm
                hover:bg-brass-light transition-colors"
            >
              Done
            </button>
      ...
      {showRotateConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-obsidian-950/80 backdrop-blur-sm">
          <div className="card-surface p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-warm-300 text-sm font-body mb-4">
              This will revoke the current key and generate a new one.
              Any integrations using the old key will immediately lose
              access.
            </p>
            <div className="flex gap-3">
              <button
              ...
              <button
                onClick={() => handleRotate(showRotateConfirm)}
                className="flex-1 py-2.5 rounded-lg bg-brass text-obsidian-950 font-body font-semibold text-sm
                  hover:bg-brass-light transition-colors"
              >
                Rotate Key
      ...
      {showRevokeConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-obsidian-950/80 backdrop-blur-sm">
          <div className="card-surface p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-warm-300 text-sm font-body mb-4">
              Are you sure? Any integrations using this key will
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-lg bg-red-500/80 text-white font-body font-semibold text-sm
