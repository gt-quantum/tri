...
export default function TenantDetailPage() {
  ...
  return <TenantDetail id={id} data={data} orgName={orgName} user={user} logout={logout} />
}

function TenantDetail({ id, data, orgName, user, logout }: any) {
            ...
            <div className="card-surface p-5">
              {parentTenant && (
                <div className="flex items-center gap-2 text-sm font-body">
                  ...
                  <Link href={`/tenant/${parentTenant.id}`} className="text-brass hover:text-brass-light transition-colors font-medium">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {subsidiaries.map((sub: any) => (
                      <Link key={sub.id} href={`/tenant/${sub.id}`} className="badge bg-brass/10 text-brass border border-brass/20 hover:bg-brass/20 transition-colors cursor-pointer">
                        {sub.company_name}
                      </Link>
                    ))}
                  </div>
                  ...
                  {activeLeases.map((lease: any) => {
                    ...
                    return (
                      <tr key={lease.id} className={`border-b border-obsidian-700/50 last:border-0 hover:bg-brass-faint/50 transition-colors ${expiring ? 'bg-amber-400/[0.03]' : ''}`}>
                        <td className="table-cell">{prop ? <Link href={`/property/${prop.id}`} className="font-semibold text-warm-white hover:text-brass transition-colors">{prop.name}</Link> : <span className="text-warm-400">Unknown</span>}</td>
                    ...
                    {expiredLeases.map((lease: any) => {
                      ...
                      return (
                        <tr key={lease.id} className="border-b border-obsidian-700/50 last:border-0">
                          <td className="table-cell">{prop ? <Link href={`/property/${prop.id}`} className="text-warm-400 hover:text-brass transition-colors">{prop.name}</Link> : <span className="text-warm-500">Unknown</span>}</td>
