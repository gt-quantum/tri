...
export default function PropertyDetailPage() {
  ...
  const { data, orgName, user, loading, logout } = useDashboardData()
  ...
  return <PropertyDetail id={id} data={data} orgName={orgName} user={user} logout={logout} />
}

function PropertyDetail({ id, data, orgName, user, logout }: any) {
  ...
  const propertySpaces = useMemo(() => {
    if (!property) return []
    return spaces.filter((s: any) => s.property_id === property.id)
  }, [property, spaces])

  const propertyLeases = useMemo(() => {
    if (!property) return []
    return leases.filter((l: any) => l.property_id === property.id)
  }, [property, leases])

  const activeLeases = useMemo(() => propertyLeases.filter((l: any) => l.status === 'active'), [propertyLeases])
  const expiredLeases = useMemo(() => propertyLeases.filter((l: any) => l.status === 'expired'), [propertyLeases])

  const today = new Date()
  const sixMonthsOut = new Date(today)
  sixMonthsOut.setMonth(sixMonthsOut.getMonth() + 6)

  const occupiedCount = propertySpaces.filter((s: any) => s.status === 'occupied').length
  const vacantCount = propertySpaces.filter((s: any) => s.status === 'vacant').length
  const occupancyRate = propertySpaces.length > 0 ? (occupiedCount / propertySpaces.length) * 100 : 0
  ...
  const appreciation =
    property && property.acquisition_price && property.current_value
  ...
  const spaceRows = useMemo(() => {
    return propertySpaces
      .map((s: any) => {
        const activeLease = leases.find((l: any) => l.space_id === s.id && l.status === 'active')
        const negotiationLease = leases.find((l: any) => l.space_id === s.id && l.status === 'under_negotiation')
        const tenant = activeLease ? tenants.find((t: any) => t.id === activeLease.tenant_id) : null
        const negotiationTenant = negotiationLease ? tenants.find((t: any) => t.id === negotiationLease.tenant_id) : null
        return { ...s, tenant, activeLease, negotiationTenant, negotiationLease }
      })
      .sort((a: any, b: any) => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="border-b border-brass-faint">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/" className="font-display text-lg text-warm-white tracking-wide hover:text-brass transition-colors">
              TRI Platform
            </Link>
            {orgName && (
              <>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={logout} className="px-3 py-1.5 rounded-lg border border-brass-faint text-warm-300 font-body text-xs hover:border-brass/20 hover:text-warm-white transition-colors">
              Log out
            </button>
          </div>
        </div>
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-8">
        <Link href="/" className="inline-flex items-center gap-2 text-warm-300 hover:text-brass text-sm font-body transition-colors mb-6">
                  ...
                  {spaceRows.map((row: any) => {
                    ...
                    return (
                      <tr key={row.id} className={`border-b border-obsidian-700/50 last:border-0 hover:bg-brass-faint/50 transition-colors group ${isVacant ? 'bg-obsidian-900/40' : ''}`}>
                        ...
                        <td className="table-cell">
                          {row.tenant ? (
                            <Link href={`/tenant/${row.tenant.id}`} className="text-warm-white hover:text-brass transition-colors">{row.tenant.company_name}</Link>
                          ) : hasNegotiation ? (
                            <Link href={`/tenant/${row.negotiationTenant.id}`} className="text-blue-400/80 hover:text-blue-400 transition-colors italic">{row.negotiationTenant.company_name}</Link>
                  ...
                  {activeLeases.map((lease: any) => {
                    ...
                    return (
                      <tr key={lease.id} className={`border-b border-obsidian-700/50 last:border-0 hover:bg-brass-faint/50 transition-colors ${expiring ? 'bg-amber-400/[0.03]' : ''}`}>
                        <td className="table-cell">
                          {tenant ? <Link href={`/tenant/${tenant.id}`} className="font-semibold text-warm-white hover:text-brass transition-colors">{tenant.company_name}</Link> : <span className="text-warm-400">Unknown</span>}
                        </td>
                    ...
                    {expiredLeases.map((lease: any) => {
                      ...
                      return (
                        <tr key={lease.id} className="border-b border-obsidian-700/50 last:border-0">
                          <td className="table-cell">{tenant ? <Link href={`/tenant/${tenant.id}`} className="text-warm-400 hover:text-brass transition-colors">{tenant.company_name}</Link> : <span className="text-warm-500">Unknown</span>}</td>
      ...
      <div className="text-center py-6">
        <p className="text-warm-500 text-[10px] font-body uppercase tracking-[0.2em]">{orgName || 'Portfolio'} &middot; Portfolio Intelligence</p>
      </div>
    </div>
  )
}
